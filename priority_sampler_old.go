...
type prioritySamplerOld struct {
	sum     uint
	samples []sampleItem
}

func (s *prioritySamplerOld) Add(priority uint, redisJobs, redisJobsInProg, redisJobsPaused, redisJobsLock, redisJobsLockInfo, redisJobsMaxConcurrency string) {
	sample := sampleItem{
		priority:                priority,
		redisJobs:               redisJobs,
...
//	~1ms for 1000 jobs
//	~4ms for 2000 jobs
func (s *prioritySamplerOld) Sample() []sampleItem {
...
func (s *prioritySamplerOld) GetSamples() []sampleItem {
	return s.samples
}
